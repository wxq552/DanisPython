...
def writeQ(dbpool,queue1):
    ...
    cursor = conn.cursor()
    cursor.execute("select order_id,stock_code,order_type from ts_gs_stock_order where order_status = 1")
...
def callApi(queue2,dbpool,queue1):
    ...
    if int(result) == 1:
        ...
        cursor.execute("select order_id,stock_code,order_type from ts_gs_stock_order where order_id = %s",[orderId]) #成交处理失败，在行进处理
