...
class MyClient(weibo.APIClient):
    def __init__(self, app_key, app_secret, redirect_uri=None, response_type='code', domain='api.weibo.com', version='2',access_token="",nickname=""):
...
#从文件中获取access_token
def load_access_token(filename = TOKEN_FILE):
        access_token_list = []
...
client = MyClient(app_key=APP_KEY, app_secret=APP_SECRET, redirect_uri=CALLBACK_URL)

def get_client(access_token="",nickname=""):
    ...
    if access_token:
        ...
        return False
    #设置获取到的access_token到client   
    """     
    access_token_list = load_access_token()
